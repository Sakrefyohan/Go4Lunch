...
public class NotificationReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        @SuppressLint("UnspecifiedImmutableFlag") PendingIntent pendingIntent = PendingIntent.getActivity(context, 152, intentNotification, PendingIntent.FLAG_UPDATE_CURRENT);
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    if (restaurantUid.equals("") && restaurantName.equals("")) {
                        contentTitle = String.valueOf(R.string.you_haven_t_chosen_a_restaurant_for_lunch_yet);
                    } else {
                        ...
                        WorkmatesHelper.getAllJoiningWorkmate(restaurantUid).addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                            ...
                            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                ...
                                for (int i = 0; i < mWorkmates.getValue().size(); i++) {
                                    ...
                                    if (i < mWorkmates.getValue().size() - 1)
                                        notificationText += ", ";
                                }
